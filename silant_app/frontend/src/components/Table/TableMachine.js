...
export const TableMachine = ({dataMachine, referenceBooks, user}) => {
        ...
        function setLocalStorageDataDetaiNode (number, list) {
            for (let item of list) {
                if (item.id === number) {
                    setLocalStorage('detail_node', item)
                    break
                }
            }   
        }
        ...
        <tbody>
            ...
            <tr className={style.table__row} key={index}>
                <td className={style.table__column}>
                    <Link className={style.link} to="/detail/technical_maintenance" onClick={()=>{setLocalStorage('number_machine_to_detail', row.number_machine)}}>
                        {row.number_machine}
                    </Link>
                </td>
                ...
                <td className={style.table__column}>
                    <Link className={style.link} to="/detail_node/" onClick={ () => {setLocalStorageDataDetaiNode(row.model_technic, referenceBooks.model_technic)}}>
                        {getDataReferenceBooks(row.model_technic, referenceBooks.model_technic)}
                    </Link>
                </td>

                <td className={style.table__column}>
                    <Link className={style.link} to="/detail_node/" onClick={ () => {setLocalStorageDataDetaiNode(row.model_engine, referenceBooks.model_engine)}}>
                        {getDataReferenceBooks(row.model_engine, referenceBooks.model_engine)}
                    </Link>
                </td>
                <td className={style.table__column}>{row.number_engine}</td>

                <td className={style.table__column}>
                    <Link className={style.link} to="/detail_node/" onClick={ () => {setLocalStorageDataDetaiNode(row.model_transmission, referenceBooks.model_transmission)}}>
                        {getDataReferenceBooks(row.model_transmission, referenceBooks.model_transmission)}
                    </Link>
                </td>
                <td className={style.table__column}>{row.number_transmission}</td>

                <td className={style.table__column}>
                    <Link className={style.link} to="/detail_node/" onClick={ () => {setLocalStorageDataDetaiNode(row.model_driving_bridge, referenceBooks.model_driving_bridge)}}>
                        {getDataReferenceBooks(row.model_driving_bridge, referenceBooks.model_driving_bridge)}
                    </Link>
                </td>
                <td className={style.table__column}>{row.number_driving_bridge}</td>

                <td className={style.table__column}>
                    <Link className={style.link} to="/detail_node/" onClick={ () => {setLocalStorageDataDetaiNode(row.model_controlled_bridge, referenceBooks.model_controlled_bridge)}}>
                        {getDataReferenceBooks(row.model_controlled_bridge, referenceBooks.model_controlled_bridge)}
                    </Link>
                </td>
